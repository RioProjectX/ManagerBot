...
def add_chat(update: Update, context: CallbackContext):
    ...
    if not is_chat:
        ...
        msg.reply_text("RioRobot AI successfully enabled for this chat!")
        message = (
        ...
        msg.reply_text("RioRobot AI is already enabled for this chat!")
        return ""
...
def remove_chat(update: Update, context: CallbackContext):
    ...
    is_chat = sql.is_chat(chat.id)
    if not is_chat:
        msg.reply_text("RioRobot AI isn't enabled here in the first place!")
        return ""
    else:
        sql.rem_chat(chat.id)
        msg.reply_text("RioRobot AI disabled successfully!")
        message = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#AI_DISABLED\n"
            f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
